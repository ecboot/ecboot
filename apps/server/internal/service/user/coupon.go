...
type IUserCouponLogic interface {
	...
	AvailableTemplates(ctx context.Context, userId int64, page PageQuery) (*PageResult[AvailableCoupon], error)
	...
	Mine(ctx context.Context, userId int64, status int, page PageQuery) (*PageResult[MyCouponItem], error)
	// UsableForOrder 结算可用券匹配（门槛<=商品金额, 按抵扣降序）。
	UsableForOrder(ctx context.Context, userId int64, goodsAmount string) ([]UsableCouponItem, error)
