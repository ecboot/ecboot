...
type IInventoryLogic interface {
	// List 库存列表（可售推导）。
	List(ctx context.Context, skuId int64, keyword string, page model.PageReq) (*model.PageResult[model.InventoryItem], error)
	// Warnings 预警列表（可售≤warn_count）。
	Warnings(ctx context.Context, page model.PageReq) (*model.PageResult[model.InventoryItem], error)
