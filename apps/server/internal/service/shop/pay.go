...
type IPayLogic interface {
	...
	Create(ctx context.Context, userId int64, orderNo string, channel int) (*model.PayCreated, error)
	// Status 状态查询。
	Status(ctx context.Context, userId int64, payNo string) (*model.PayStatus, error)
