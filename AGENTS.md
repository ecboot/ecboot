...
ECBOOT is an e-commerce platform monorepo in early scaffold stage: a **Go + GoFrame v2** backend at `apps/server` plus four frontend apps, all in a Vite+ / pnpm workspace (Node >= 22.18, pnpm). The backend is a modular monolith (`internal/{web,api,service,common,infra}` + `dao/model` generated by `gf gen dao`); database schema is mature (70 tables via golang-migrate migrations 000001~000031, see `docs/schema-design.md`). Spec Kit scaffolding lives in `.specify/` (constitution v2.0.0: 模块化单体 / 统一技术栈 / 中文优先 / 可验证交付 / 简单优先).

## Repository Layout

- `apps/server/` — the Go backend (GoFrame v2, single module `ecboot`): `main.go` (assembly) + `internal/` layered packages:
  - `internal/api/{user,shop,admin,commonc}` — four mutually-isolated channel packages (member center / storefront / admin console / public APIs: SMS & image captcha); they depend on `internal/web` — the shared web foundation (unified response / exception handling / auth).
  - `internal/service/{user,shop}` — domain packages (sibling-isolated, cooperate via domain events).
  - `internal/common` / `internal/infra` — foundation layers (shared objects / technical components: captcha, SMS, crypto, sessions).
  - `internal/dao` + `internal/model` — `gf gen dao` generated artifacts; NEVER hand-edit (regenerate via `make gen`).
...
make lint           # golangci-lint run    (includes depguard layer enforcement)
...
Dependency rules are enforced by **depguard** in `.golangci.yml` — violations fail `make lint`:

- Layer direction: `main → internal/api/* → internal/service/* → internal/common | internal/infra`; the four channels (`api/commonc|user|shop|admin`) never import each other; `api/commonc` additionally must not import any `service/*`; `internal/web` is a pure technical foundation (no channel/domain imports); domain siblings (`service/user` ↔ `service/shop`) never import each other.
...
Database migrations live in `apps/server/migrations/` (golang-migrate format `NNNNNN_name.up/down.sql`, 000001~000031, 70 tables). Schema design rationale: `docs/schema-design.md`.
...
`apps/server` is a Go modular monolith: `internal/common` / `internal/infra` (foundation) ← `internal/service/{user,shop}` (domains) ← `internal/api/{commonc,user,shop,admin}` (channels, sharing `internal/web` as the web foundation) — assembled by `main.go` into one deployable. The dependency wiring IS enforced by depguard (see Backend commands above).
