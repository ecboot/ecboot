...
| 2026-09-21 | 02 | 记录在案修正口径：**任何 DB 错误**（非仅非法编码）经 response.mapError 暴露为 code 52 且绕过 g.Log 与追踪号 | 评审 I4 实证：`gc.Code()>0` 提前返回致 10002 语义失效；影响面为批次 01/02 全部 DB 错误路径，单开 chore 批修 mapError 一处 |
| 2026-09-21 | 02 | 评审修复轮（With fixes）：C1 status 白名单 / I1 全量覆盖护栏 / I2 坐标清零显式置空 / I3 分页越界 / I6 idgen 机器码回退 / M2/M4/M6 | 独立评审实证复现；I5（IStoreLogic 无实现者）与 I4 因涉既有跨批次形态，记为待裁定 | |
