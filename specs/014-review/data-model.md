...
| 动作 | 前置条件 | 效果 | 失败码 |
|---|---|---|---|
| Create | 订单项属于本人 且 所属订单**已完成** 且 未评过 | 落评价（audit_status=1） | 未归属→不存在（10006 `CodeNotFound`）；**未完成/已取消→40006（状态不允许）**；已评过→40010（`CodeAlreadyReviewed`，含唯一键 1062 转换）；评分越界→10001 |
