...
- **实现**: `dashboard_impl.go`（Trade/Member/Product 三方法, 口径全部锚定既有域查询: 已支付枚举 **20/30/40**（排除 90 已取消）销售额 pay_amount 合计/退款额售后完成 status=50 实退/休眠 last_active_at≥**配置阈值**（读 system_config `dormant.tier1.days`, 与 user/wx.go 同源; 休眠定义在 **000027**）/低库存 available<=warn_count 复用 inventory 预警口径/待审核评价 audit_status=0）; 连线 3 桩
