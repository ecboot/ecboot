...
- **待裁定（跨批次形态, 不在本批）**:
  - **I4（本批已修, 前移理由见下）**: response.mapError 判据改为契约码域 `>=10001`——框架码 51/52 归一为 10001/10002, 恢复字段级校验文案与系统错误日志+追踪号。
    前移理由: 本批 C1/I1 的修复本意即"拒绝并给出可读错误", 而框架码直出使其实际返 51——不修则本批验收不成立。HTTP 复验: status=0 → 10001 含字段细节 ✓
