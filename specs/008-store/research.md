...
## D6 门店修改 = 全量覆盖语义（实现期决策, 记账）

**结论**：`AdminUpdate` 采用全量覆盖（后台表单提交完整档案）+ 显式 `Fields` 白名单写入。

**理由**：gf 的 `do` 结构体带 `omitempty`，零值字段默认被吞——而"关闭自提开关（false）""置歇业"
必须能写零值。显式 `Fields(白名单)` 可强制写入（已用测试验证：关闭自提断言通过）。
全量覆盖同时消除 bool 字段"未传 vs 传 false"的二义性（表单语义天然完整提交）。
`Insert` 无需白名单（零值 = 建表默认值, 语义一致）。

**替代方案**：部分更新（非零字段才写）——bool 无法表达"关闭"; 指针化 DTO（*bool）——api 契约改动大; 均放弃。
